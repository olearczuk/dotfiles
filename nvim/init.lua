...
require('lazy').setup({
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('gdt', '<cmd>tab split | lua vim.lsp.buf.definition()<CR>', '[G]oto [D]efinition in new [T]ab')
          map('gdv', ':vsplit | lua vim.lsp.buf.definition()<CR>', '[G]oto [D]efinition in [V]ertical split')
          map('gdx', ':split | lua vim.lsp.buf.definition()<CR>', '[G]oto [D]efinition in new horizontal split')
          ...
          map('gIt', '<cmd>tab split | lua vim.lsp.buf.implementation()<CR>', '[G]oto [I]mplementation in new [T]ab')
          map('gIv', ':vsplit | lua vim.lsp.buf.implementation()<CR>', '[G]oto [I]mplementation in new [V]ertical split')
          map('gIx', ':split | lua vim.lsp.buf.implementation()<CR>', '[G]oto [I]mplementation in new horizontal split')

          -- Jump to the type of the word under your cursor.
          --  Useful when you're not sure what type a variable is and you want to see
          --  the definition of its *type*, not where it was *defined*.
          map('<leader>D', require('telescope.builtin').lsp_type_definitions, 'Type [D]efinition')
          map('<leader>Dt', '<cmd>tab split | lua vim.lsp.buf.type_definition()<CR>', 'Type [D]efinition in new [T]ab')
          map('<leader>Dv', ':vsplit | lua vim.lsp.buf.type_definition()<CR>', 'Type [D]efinition in [V]ertical split')
          map('<leader>Dx', ':split | lua vim.lsp.buf.type_definition()<CR>', 'Type [D]efinition in new horizontal split')
          ...
          map('gDt', '<cmd> tab split | lua vim.lsp.buf.declaration()<CR>', '[G]oto [D]eclaration in new [T]ab')
          map('gDv', ':vsplit | lua vim.lsp.buf.declaration()<CR>', '[G]oto [D]eclaration in [V]ertical split')
          map('gDx', ':split | lua vim.lsp.buf.declaration()<CR>', '[G]oto [D]eclaration in horizontal split')
    ...
    config = function()
      ...
      local function open_output_panel()
        ...
        for _, win in ipairs(vim.api.nvim_list_wins()) do
          local ok, name = pcall(vim.api.nvim_buf_get_name, vim.api.nvim_win_get_buf(win))
          if ok and name:find('Neotest Output Panel', 1, true) then
      ...
      require('neotest').setup {
        ...
        output_panel = {
          open = function()
            local tree_winid = require('nvim-tree.api').tree.winid()
            local tree_width = tree_winid and vim.api.nvim_win_get_width(tree_winid) or 0
            local width = math.floor((vim.o.columns - tree_width) / 2)
            vim.cmd('vertical ' .. width .. 'sp')
            return vim.api.nvim_get_current_win()
          end,
        },
        discovery = {
          filter_dir = function(name, _, _)
            return name ~= '.git' and name ~= 'build' and name ~= 'out'
          end,
        },
        adapters = adapters,
      }
  ...
  { -- CMake project support: configure/build/run + compile_commands.json for clangd
    ...
    config = function(_, opts)
      ...
      local function focus_cmake_terminal(attempts)
        ...
        if attempts > 0 then
          vim.defer_fn(function()
            focus_cmake_terminal(attempts - 1)
          end, 50)
        end
      end
      ...
      vim.keymap.set('n', '<leader>cmr', function()
        local tree_winid = require('nvim-tree.api').tree.winid()
        local tree_width = tree_winid and vim.api.nvim_win_get_width(tree_winid) or 0
        require('cmake-tools.const').cmake_runner.opts.split_size = math.floor((vim.o.columns - tree_width) / 2)
        vim.cmd 'CMakeRun'
        focus_cmake_terminal()
